(() => { /* Immediately Invoked Function Expression (IIFE): this function runs right away when the script loads. Wrapping everything in an IIFE keeps our variables out of the global scope so they don't clash with other scripts. */
  ...
  const FOOD_TYPES = [ /* An array of food options shown in the food step. Each object has an emoji and a label. */
    ...
    { emoji: "🥩", label: "Chili's" },
    { emoji: "🍕", label: "Pizza Hut" },
    { emoji: "🍔", label: "Burger King" },
    { emoji: "🦐", label: "Orange Bucket" },
    { emoji: "🍝", label: "Italian Restaurant" },
    { emoji: "🍜", label: "Ramen Restaurant" },
    { emoji: "🍣", label: "Sushi Restaurant" },
    { emoji: "💗", label: "Me" }
  ];
  ...
  const BG_ITEMS = [ /* An array of all possible floating background items. Each has an emoji and a CSS class to apply. */
    ...
    { emoji: "💕", cls: "heart" }, /* Two hearts */
    { emoji: "🌸", cls: "sparkle" }, /* Cherry blossom */
    { emoji: "💫", cls: "sparkle" }, /* Dizzy star */
    { emoji: "🎀", cls: "balloon" }, /* Ribbon bow */
  ...
  const NO_CRIES = [ /* An array of sad messages that appear in order (or cycling) when the user presses "No" repeatedly — the messages STILL say "No" but are pleading/dramatic */
    ...
    "Really? You’re making me blush over here 😔", /* Message shown the 3rd time */
    "Still No? Aww, give me a chance! ☹️", /* Message shown the 4th time */
    "Okay… but I’m not giving up yet 😛", /* Message shown the 5th time */
    "Another No?! You’re making this difficult 😣", /* Message shown the 6th time */
    "Come onnn, I know you want to say yes! 💖", /* Message shown the 7th time */
    "One more chance? Pretty please? 🥺", /* Message shown the 8th time */
    "Okay, final question… will you say yes? 💕"/* Message shown for the 9th press and reused for all later presses (last one) */
  ];
  ...
  function cryBurst(x, y) { /* Defines a function that spawns a burst of tear/emoji particles at a given position (x, y) */
    const tears = ["😔", "💧", "☹️", "😭", "💔", "😣", "🥺"]; /* An array of tear and sad emojis that will appear in the burst */
